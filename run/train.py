...
def main(args):
    ...
    train_dataset = CustomDataset("resource/data/국회회의록안건별요약_train.json", tokenizer)
    valid_dataset = CustomDataset("resource/data/국회회의록안건별요약_dev.json", tokenizer)
    ...
    class MemoryClearCallback(TrainerCallback):
        def on_step_end(self, args, state: TrainerState, control: TrainerControl, **kwargs):
            # 매 10 스텝마다 메모리 캐시 비우기
            torch.cuda.empty_cache()
            print(f"Step {state.global_step}: GPU memory cache cleared.")
